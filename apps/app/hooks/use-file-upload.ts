...
export const useFileUpload = (
  ...
  const validateFileCount = useCallback(
    (newFilesLength: number): string | null => {
      if (
        multiple &&
        maxFiles !== Number.POSITIVE_INFINITY &&
        state.files.length + newFilesLength > maxFiles
      ) {
  ...
  const addFiles = useCallback(
    (newFiles: FileList | File[]) => {
      ...
      const countError = validateFileCount(newFilesArray.length);
      if (countError) {
      ...
      updateFilesState(validFiles, errors);
    ...
    [clearFiles, validateFileCount, processFile, updateFilesState, multiple]
  );
